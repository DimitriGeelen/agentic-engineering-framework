...
description: >
  Create tests/e2e/ directory structure with runner.sh orchestrator, lib/setup.sh
  (spawn TermLink session in temp dir with fw init), lib/teardown.sh (cleanup), lib/assert.sh
  (file_exists, exit_code_is, grep_file, yaml_field helpers). From T-513 inception
  build task 1.
