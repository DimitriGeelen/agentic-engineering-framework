...
horizon: null
tags: []
components: [bin/fw, lib/hook-parity.sh]
related_tasks: []
...
last_update: 2026-08-20T21:53:24Z
date_finished: 2026-08-20T21:53:24Z
...
### 2026-08-20T17:36:32Z — task-created [task-create-agent]
...
- **Scan ID:** R-17eaa369
- **Timestamp:** 2026-08-20T21:59:23Z
- **Catalogue:** v1.3-seed
- **Overall:** CONCERN
- **Needs Human:** no
- **Findings:** 2
...
- **AC#3 (Agent)** — A linked worktree whose `.claude/settings.json` is missing hooks the authority has → WARN naming the worktree and the missing hook names.
  - **AC-verify-mismatch** (narrow, heuristic) — `path=claude/settings.json in: A linked worktree whose `.claude/settings.json` is missing hooks the authority has → WARN naming the worktree and the missing hook names.`
- **AC#4 (Agent)** — A linked worktree with no `.claude/settings.json` at all → WARN (`settings absent`), never silent.
  - **AC-verify-mismatch** (narrow, heuristic) — `path=claude/settings.json in: A linked worktree with no `.claude/settings.json` at all → WARN (`settings absent`), never silent.`

### 2026-08-20T21:53:24Z — status-update [task-update-agent]
