---
id: T-2568
name: "832 pair-draft #2 dispatch-loop byte-exact + compile (parallelGateway probe)"
description: >
...
# T-2568: 832 pair-draft #2 dispatch-loop byte-exact + compile (parallelGateway probe)

## Context

832 delivered pair-draft #2 (`dispatch-loop.bpmn`, their Sub-Agent Dispatch Protocol) at rail offset 96 — pinned sha256 `95bc24cdb0d27952a4f85da55368b74fc8c1e9586960d0dd839453595543594b`, 18793 B. The offset-96 base64 blob was an explicit truncated PREVIEW (3084 B decoded, sha mismatch, truncation marker in tail); full blob re-requested per 832's own instruction. Fresh probes this diagram carries that the corpus has NOT: (1) balanced parallelGateway fork/join (3-way), (2) two back-edges converging on one node (agt_2_scope has 3 incoming), (3) exclusiveGateway branch opening a parallel region, (4) labeled exclusive branches vs deliberately unlabeled parallel-fork edges. Also 2 Framework·Authority nodes → expected T-2567-class silent agent-fold (current behavior). Sibling of T-2566 (pair-draft #1).
...
test "$(stat -c%s tests/fixtures/aef-bpmn/dispatch-loop.bpmn)" = "18793"
out=$(sha256sum tests/fixtures/aef-bpmn/dispatch-loop.bpmn); echo "$out" | grep -q "95bc24cdb0d27952a4f85da55368b74fc8c1e9586960d0dd839453595543594b"
python3 -m pytest tests/unit/test_bpmn_to_tasks.py -q 2>&1 | tail -1 | grep -q "passed"
test -s docs/reports/T-2568-pairdraft2-compile.md
...
### 2026-07-19T21:28:57Z — task-created [task-create-agent]
