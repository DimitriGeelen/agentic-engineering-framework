...
description: >
  fw doctor FAILS on every new project with 'script not found'. Root cause: bin/fw:392
  uses command.split()[0] to extract script path from hook commands, but grabs PROJECT_ROOT=...
  env var assignment instead of the actual script path. Fix: skip tokens containing
  '=' when extracting script path. Source: T-294 simulation O-003.
