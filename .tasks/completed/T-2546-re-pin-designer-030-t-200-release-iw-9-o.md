...
last_update: 2026-07-19T08:04:27Z
date_finished: null
...
**Blocked on clean delivery:** first `file_send` (xfer-mcp-3273116) failed reassembly
("17/1 chunks"); re-fire requested from 832 on the rail (offset 66 reply).

## Acceptance Criteria

### Agent
- [ ] 0.3.0 artifact delivered clean: `file_receive` reassembles + integrity-passes; landed file sha256 == pin (`36be033d…`), bytes == 826643
- [ ] `policy/designer-pin.yaml` bumped to 0.3.0 (version, sha256, bytes, vendored_path, source_artifact, source_tag; cdn_fonts→false per offline-hardening)
- [ ] `fw designer sync --from <delivered>` installs read-only to `vendor/designer/aef-workflow-designer-0.3.0.html`; `fw designer status` reports PRESENT ✓ (sha256 matches pin)
- [ ] `fw doctor` clean on designer pin drift (no `designer pin sha256 drift` WARN)
- [ ] sha256 match confirmed back to 832 on the rail BEFORE sync (offset-64 commitment honored)
