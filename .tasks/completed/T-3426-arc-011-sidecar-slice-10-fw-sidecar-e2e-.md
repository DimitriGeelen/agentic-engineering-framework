...
horizon: null
tags: []
components: [agents/context/lib/safe-commands.sh, agents/git/lib/commit.sh, lib/sidecar_cli.py, lib/sidecar/e2e.py, lib/sidecar/termlink_transport.py, tests/unit/test_sidecar_e2e.py, tests/unit/test_sidecar_termlink_transport.py]
related_tasks: []
...
- [x] **Live, two-party — ANSWERED, 73 hours after the window closed. Round trip PROVEN; the FAIL was the instrument, not the rail:** run `ab947312` (`.context/sidecar/e2e/ab947312.json`, committed): H1 PASS, H2 PASS (our consult on `sidecar:010-termlink` @1), H4/H5 FAIL — no answer from 010-termlink within 1802 s (their session had not read the topic in the window; not a transport fault: H2 proves the hub holds it). Re-run on their signal per the resume note below. Originally: sent 10:20Z — H1 our ledger INJECTED_NOW; H2 the consult is on `sidecar:010-termlink` @1 (client_msg_id in RPC param + metadata + cv_key, conversation `e2e-ab947312`, from `e2e-ab947312-sender`); the request with the one-line reply command posted to TermLink on agent-chat-arc @1651 and on their DM thread @8. Harness window closes ~10:50Z. **Outcome to record on resume:** read `.context/sidecar/e2e/ab947312.json` — if H4/H5 PASS, tick with their offsets; if FAIL (no answer in the window), keep the record, and when TermLink signals they read it re-run `bin/fw sidecar e2e --peer 010-termlink --task T-3426` and record that run. Their mirror (a consult to `sidecar:999-Agentic-Engineering-Framework`) answers itself through the T-3407 hook. **RESOLVED 2026-09-25 (no re-run needed — the original run was answered):** `sidecar:e2e-ab947312-sender` @0 carries `SIDECAR-E2E-ACK ab947312 — from 010-termlink`, token and peer name both validated against the run record. Sent `2026-09-22T10:14:32Z`, run closed `10:44:32Z` after 120 polls, **ACK arrived `2026-09-25T11:19:35Z` — 73h 05m end to end.** So H4/H5 PASS on the evidence and the two-party round trip is proven in both directions on a real peer we do not control. What the recorded FAIL actually measured was the 1800 s window, not the rail. Two things came back with it: (a) `fw sidecar` **does not exist in their vendored build** (AEF v1.6.29 — `Unknown command: sidecar`); they answered via the raw-channel fallback our own message supplied, which worked first try, so the prescribed verb was the gap and the fallback is what saved the run; (b) they name two sibling instances of the same shape — `fw integrate run` and `checkpoint.sh budget`. Verified in our v1.7.110: all three verbs exist, so their report is **version skew from vendoring lag, not absence** — except `lib/integrate.py:12`, whose docstring still calls the mutating verb "a later slice" though `cmd_run` has been implemented for some time, and that stale line is what led them to conclude it was never built
...
### 2026-09-25 — the peer answered 73 hours late, and the harness had already called it FAIL

- **What changed:** the slice was built assuming a peer round trip is *slow but
  bounded* — hence 1800 s / 15 s polls, "long by default". The real number for a
  peer we do not control was **73h 05m**. The window was not conservative, it was
  off by two orders of magnitude. The rail held the message durably across three
  days and delivered the answer; the instrument had long since recorded FAIL.
- **Plan impact:** `FAIL` is the wrong verdict for peer mode. It conflates *"the
  peer will not answer"* with *"the peer has not answered yet"* — the same
  conflation Amendment 1 (T-3396) resolved for the ack states by inventing
  `UNKNOWN`, and the same one `INJECTED_NOW` makes at the other end of this
  system. A timeout in peer mode should be a third, non-error terminal state that
  a later read can settle, not a failure. Filed separately; not patched here,
  because changing a verdict's meaning is a design change, and this slice's ACs
  are about the harness existing, not about its calibration.
- **Triggered:** the peer's answer also carried a defect report we did not ask for
  and could not have produced ourselves — `fw sidecar` does not exist in their
  vendored v1.6.29, so the verb our own message prescribed was unusable, and only
  the raw-channel fallback we happened to include alongside it made the answer
  possible. **A one-line fallback in a message body is the only reason this slice
  has a result at all.** They named two siblings (`fw integrate run`,
  `checkpoint.sh budget`); both exist in our v1.7.110, so the class is vendoring
  lag rather than absence — with one genuine exception, the stale
  `lib/integrate.py:12` docstring that still calls the mutating verb "a later
  slice" and is what led them to conclude it was never built.
...
- **Scan ID:** R-e3027ea9
- **Timestamp:** 2026-09-25T19:47:52Z
- **Catalogue:** v1.3-seed
- **Overall:** CONCERN
- **Needs Human:** no
- **Findings:** 2
...
- **AC#4 (Agent)** — **Live, two-party — ANSWERED, 73 hours after the window closed. Round trip PROVEN; the FAIL was the instrument, not the rail:** run `ab947312` (`.context/sidecar/e2e/ab947312.json`, committed): H1 PAS
  - **AC-verify-mismatch** (narrow, heuristic) — `path=context/sidecar/e2e/ab947312.json in: **Live, two-party — ANSWERED, 73 hours after the window closed. Round trip PROVEN; the FAIL was the instrument, not the rail:** run `ab947312` (`.cont`

**Verification-level findings:**

  1. **mock-only-integration** (partial, heuristic) @ AC vs Verification cross-check
     - evidence: `python3 -m pytest tests/unit/test_sidecar_e2e.py tests/unit/test_sidecar_termlink_transport.py tests/unit/test_sidecar_sweep.py tests/unit/test_sidecar_status.py tests/unit/test_sidecar_inbox.py tests`
