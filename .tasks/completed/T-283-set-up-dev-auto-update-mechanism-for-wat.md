...
description: >
  Configure automatic updates for watchtower-dev on the LXC: OneDev webhook on push
  to master triggers git pull + systemctl restart watchtower-dev on the LXC. Alternative:
  cron job every 5 minutes checking for new commits. Prod updates remain manual/explicit
  only. Depends on T-281 (services deployed). See T-279 research. Parent: T-279.
