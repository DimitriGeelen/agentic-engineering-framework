---
id: T-3008
name: "raise shared Ollama to MAX_LOADED_MODELS=2 and restart (operator-authorised)"
description: >
...
Operator-authorised follow-on to T-3006. That task deliberately did **not** touch
the shared Ollama service (its AC #6 promised so) because three other fleet hosts
depend on it — `.170`, `.171`, `.129`. The operator has now decided; this task
carries the change so T-3006's promise stays true.

Root cause being addressed: `OLLAMA_MAX_LOADED_MODELS=1` in
`/etc/systemd/system/ollama.service.d/parallel.conf` means one model slot
fleet-wide. A chat model on a continuously-renewed 30m lease starves every other
model — measured, the loaded model answered 200/0.87s while any unloaded model
503'd in 0.17s. Raising to 2 lets the embedding model co-reside.

VRAM: 16.3 GB total, gemma4 alone 11 GB, embedding models 0.6–1.0 GB — fits.

## Acceptance Criteria

### Agent
- [x] `OLLAMA_MAX_LOADED_MODELS=2` is set in the systemd drop-in and survives a
      restart (not just an env poke), with the prior file backed up so the change
      is revertible in one copy.
- [x] Service restarted and `active (running)`.
- [x] **Embedding works on the shared host** (`:11434`) — the thing that was
      broken. This is the point of the change, not the restart.
- [x] **Generation works** (`qwen3:14b` on the shared host) — proves the second
      slot resolves the starvation for chat too, not only for embeddings.
- [x] Two models confirmed resident simultaneously via `/api/ps` — direct
      evidence the slot count actually took effect, rather than inferring it from
      one request succeeding.
- [x] The full briefing path renders: `lib/ask.py` returns non-empty output
...
### 2026-08-15T06:05:44Z — task-created [task-create-agent]
