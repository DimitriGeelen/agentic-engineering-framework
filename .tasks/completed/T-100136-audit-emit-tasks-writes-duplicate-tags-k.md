---
id: T-100136
name: "audit emit-tasks writes duplicate tags: key — emitter tags silently discarded
  (YAML last-key-wins)"
description: >
  Emitted finding tasks carry 'tags: [audit-finding, severity:X, section:Y]' from
  the emitter AND the template's 'tags: []' line survives below it. yaml.safe_load
  takes the last occurrence, so the emitter's tags never take effect (and the file
  is invalid strict-YAML). Fix in _emit_findings_as_tasks frontmatter injection: replace
...
# T-100136: audit emit-tasks writes duplicate tags: key — emitter tags silently discarded (YAML last-key-wins)
...
### 2026-07-04T09:26:30Z — task-created [task-create-agent]
