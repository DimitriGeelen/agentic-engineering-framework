...
Build slice A from [[T-2125]] (GO on A+B+D). Render-side forgiveness: when the agent (or a stale chat URL, or copy-paste) hits `/review/<inception-id>`, the blueprint detects the target task's `workflow_type: inception` and 302-redirects to the class-correct `/inception/<id>`. Paired with T-2129 (codification, shipped) and T-2125 narrative (backlog re-issued).

Idempotent, no behavioural change for legit `/review/<task-id>` partial-completes. Build tasks still render at `/review/<id>` because their `workflow_type` is `build`/`refactor`/`test`/etc., not `inception`.

## Acceptance Criteria

### Agent
- [x] `web/blueprints/review.py` `/review/<task_id>` handler checks the target task's `workflow_type` early; if `inception`, returns 302 redirect to `/inception/<task_id>`
- [x] `curl -sI -o /dev/null -w "%{http_code} %{redirect_url}" $URL/review/T-2123` returns `302` and a redirect_url ending in `/inception/T-2123` (T-2123 is an inception)
- [x] `curl -sI -o /dev/null -w "%{http_code}" $URL/review/T-2127` returns `200` (T-2127 is a build task — must NOT redirect)
...
# T-2131: redirect contract.
url=$(bin/fw watchtower url); code=$(curl -sI -o /dev/null -w "%{http_code}" "$url/review/T-2123"); test "$code" = "302"
url=$(bin/fw watchtower url); rdir=$(curl -sI -o /dev/null -w "%{redirect_url}" "$url/review/T-2123"); echo "$rdir" | grep -q "/inception/T-2123"
url=$(bin/fw watchtower url); code=$(curl -sI -o /dev/null -w "%{http_code}" "$url/review/T-2127"); test "$code" = "200"
...
### 2026-05-30T22:02:43Z — task-created [task-create-agent]
