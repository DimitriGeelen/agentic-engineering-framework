---
id: T-3047
name: "Triage the 39 recovered upstream messages against current framework code"
description: >
...
T-3046 shipped the router that says which recovered hub messages a human must see.
Running it against the live archive surfaces **39 unique actionable messages** —
`routed` (12) or `surfaced` (27) — spanning 2026-04-28 to 2026-07-06. Most are
upstream bug reports filed *against this framework* by consumer projects
(`proxmox-ring20-management`, `ring20-dashboard`, `002-Claude-Partner-Network`,
`025-WokrshopDesigner`). They sat unread for one to four months because the
federation delivered them into topics nobody drained.

This task does not fix them. It answers one question per message: **is the defect
still live in current framework code?** Age is not evidence of repair — several of
these describe classes the framework has since hardened, and several describe code
paths that have not been touched since the report was filed. Both need proving, not
assuming.

Scope fence: triage and file. Fixes are separate tasks, one per bug (Task Sizing
Rules). No message is actioned on the operator's behalf — the `routed` twelve
remain gated behind T-3046's open Human AC.
...
### 2026-08-16T22:20:24Z — task-created [task-create-agent]
