...
last_update: 2026-06-21T12:01:13Z
date_finished: null
...
- [ ] AC1 — `fw doctor --quick` runs project-scoped checks only, skipping host + network probes; flag is
      documented in `fw doctor --help` / usage.
- [ ] AC2 — `--quick` is measurably faster than full doctor (assert it omits the host/network sections;
      ideally a wall-clock delta in the test or a skipped-section marker).
- [ ] AC3 — full `fw doctor` (no flag) is unchanged; existing doctor tests stay green; a new assertion
      pins `--quick` skipping the host/network blocks.
