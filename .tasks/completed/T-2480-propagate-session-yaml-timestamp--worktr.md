---
id: T-2480
name: "propagate session YAML-timestamp + worktree fixes to consumers (vendor refresh
  + land handoff)"
description: >
  Refresh this repo's vendored .agentic-framework/ copies of the session's framework
  edits (integrate.py union resolver, bvp.sh + estimator.py YAML-timestamp fallback)
  so the branch can push and consumers inherit correct copies on fw upgrade. Identify
  which hosts/consumers are actually affected and give concrete per-host upgrade actions.
  Do NOT remote-upgrade machines from this session.
