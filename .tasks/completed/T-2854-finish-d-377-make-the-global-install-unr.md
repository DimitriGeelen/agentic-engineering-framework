---
id: T-2854
name: "finish D-377: make the global install unreachable — delete router fallback,
  upgrade's global sync, and the now-dead git update path"
description: >
...
# T-2854: finish D-377: make the global install unreachable — delete router fallback, upgrade's global sync, and the now-dead git update path

## Context

D-377 (2026-08-04, T-2793) decided total isolation: everything vendored
per-project, `fw` on PATH is a thin router that walks up from cwd to the nearest
`.agentic-framework/bin/fw`. T-2800/T-2809 removed the *producer* of global
installs — `install.sh` now clones to `mktemp -d` and deletes it.

What was never done is deleting the *consumers* of a global install. They remain,
each guarded by `[ -d "$global_dir" ]`, so a pre-T-2800 residue directory stays
alive and in use:

| Site | Behaviour |
|------|-----------|
| `bin/fw-router:202` | routes to the global when no project is found |
| `lib/upgrade.sh:1492` | **syncs into the global on every `fw upgrade`** — the drip that keeps it at 341MB |
| `lib/update.sh:_do_update_git` | only reachable via a global install |
| `bin/fw:1646`, `bin/fw:2392` | detection / doctor WARN — remediation aids, keep |

The router's own 127 message already asserts *"There is no global install to fall
back on by design — T-2800/D-377"*, which is only printed when no global exists.
When one does, the code twenty lines above contradicts it. That contradiction is
what surfaced as T-2853 (`fw update` misrouting) and as the operator's confusion
this session.

**Operator extension (2026-08-07):** the same principle must hold for the second
entry point. `claude-fw` should resolve per-project — walking up from cwd to the
project root and using *that* project's copy — exactly as `fw` does. A fixed copy
on PATH that always runs the same bytes regardless of which project you are in is
the global-install problem wearing a different name.
...
### 2026-08-07T10:55:42Z — task-created [task-create-agent]
