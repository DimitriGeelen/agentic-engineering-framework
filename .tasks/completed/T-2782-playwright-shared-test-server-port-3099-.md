---
id: T-2782
name: "Playwright shared test server (port 3099) never recycled — accumulates state,
  degrades readings"
description: >
  conftest.py:watchtower_server reuses any already-listening server on FW_TEST_PORT
  (3099) across sessions with no staleness/age bound and no teardown when reused.
  T-2777 traced a reproducible 'contention' misdiagnosis to this: a server process
  running unmanaged since 2026-08-03 (656MB RSS, 66min accumulated CPU) served /approvals,
  /approvals/content, /metrics at 13-16s per request; killing it and letting a fresh
  instance start dropped the same routes under 3s and the full 54-route suite from
  ~9min/51-passed to 4min/54-passed. Add a staleness bound (e.g. max process age,
  or periodic forced-recycle) to watchtower_server so the fixture cannot silently
...
# T-2782: Playwright shared test server (port 3099) never recycled — accumulates state, degrades readings
...
### 2026-08-03T23:25:34Z — task-created [task-create-agent]
