---
id: T-3089
name: "Lint: forbid backticks in quoted bash strings (T-3086 bug class)"
description: >
...
cost_estimate_proposed:
  - ts: '2026-08-19T20:00:07Z'
    ...
    rationale: blast_radius=? (no-components-UNMEASURED-not-zero); tier=2 
      (workflow:build); effort=8 (lines=140,acs=9)
    rubric_sha: e4a00f38e801
bvp_scores_proposed:
  - ts: '2026-08-19T20:00:15Z'
    estimator: bvp-estimator-v1-heuristic
    scores:
      D1: 4
      D2: 0
...
# T-3089: Lint — forbid backticks in quoted bash strings

## Context

T-3086 was a fork bomb caused by Markdown-style backticks inside a double-quoted
`FW_CONFIG_REGISTRY` element in lib/config.sh. bin/fw sources config.sh on every
invocation, so every `fw` call executed the backticked command, which sourced
config.sh again. Four kernel OOM crashes of the host in 22 hours.

The sibling guard tests/lint/no-backticks-in-inline-python.bats (T-2707) already
covered this defect shape — but only inside double-quoted `python3 -c "..."` blocks.
A backtick in a plain bash string was unguarded. This closes that half (Level C).

Detection uses shellcheck's SC2006 rather than a hand-rolled scanner: the first draft
of this guard was a bash state machine and false-positived 25 times on apostrophes in
comments, heredocs, and Python files carrying a .sh extension. shellcheck parses the
real grammar. Measured at authoring time: 199 tracked bash files, zero SC2006
violations — the guard starts green.

A second, narrower assertion (pure Python, runs without shellcheck) checks that
registry descriptions contain no backticks, `$(` or `${`, since all three expand on
source and the next instance is as likely to be a `$(...)` as a backtick.

## Acceptance Criteria

### Agent
- [x] tests/lint/no-backticks-in-quoted-strings.bats exists and all 5 tests pass
- [x] SC2006 guard scans tracked bash sources by shebang, excluding .agentic-framework/ and worktrees/
- [x] Guard fails loudly (exit 3) rather than silently passing if no files match — not vacuous
- [x] Registry assertion flags backticks, `$(` and `${` in FW_CONFIG_REGISTRY descriptions
- [x] Negative control: a planted backtick in a registry entry IS flagged
- [x] Negative control: a backtick inside a comment is NOT flagged (no false positive)
- [x] Negative control: shellcheck catches the exact T-3086 shape end to end
- [x] Zero pre-existing violations in the tree (guard is enforceable immediately)
- [x] SEE-ALSO comment cross-references tests/lint/config-registry-parity.bats

## Verification

# The guard suite is the gate: test 1 already sweeps the whole tree for SC2006,
# tests 3-5 prove it is not vacuous. Running the sweep again here would just
# double a ~2 minute shellcheck pass for no extra signal.
bats tests/lint/no-backticks-in-quoted-strings.bats
...
### 2026-08-19T19:44:38Z — task-created [task-create-agent]
