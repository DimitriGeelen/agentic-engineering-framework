...
horizon: null
tags: []
components: [tools/probe_handover_recall.py]
related_tasks: []
...
last_update: 2026-08-16T10:03:43Z
date_finished: 2026-08-16T10:03:43Z
...
T-3031 measured the corpus and named the next reduction: a **handover retention
window**. Handovers are 67.6% of the indexed corpus (96,335,564 B across 1,719
files); keeping only the last 90 would cut the corpus 55.2%, and indexing none
would cut it 67.6%. It is subtraction of the cheapest kind — no file is
rewritten or deleted, only which subset `collect_files()` returns.

But the case for it rests on a **claim about recall that has not been tested**:
that an old handover's unique retrieval value is near zero because its durable
content is restated in task files, episodics, decisions and learnings, and
because the recovery path reads `LATEST.md` rather than the archive.

That claim is plausible, which is exactly why it needs checking. Shipping a
55% corpus cut on a plausible-sounding assumption is the shape of mistake that
is invisible until someone's `fw recall` silently stops finding the thing they
needed — a *quiet* regression, with nothing going red.

So this task produces the evidence, and only the evidence. It does not
implement the window.

**Scope fence:** no change to `collect_files()`, no new config key, no reindex
strategy change. If the evidence says the window is safe, the implementation is
a separate task; if it says otherwise, the alternative (retro-digesting the
stock, which is a migration) is also a separate task.
...
### 2026-08-16T09:57:44Z — task-created [task-create-agent]
- **Action:** Created task via task-create agent
- **Output:** /opt/999-Agentic-Engineering-Framework/.tasks/active/T-3032-do-old-handovers-actually-get-retrieved-.md
- **Context:** Initial task creation

## Reviewer Verdict (v1.5)

- **Scan ID:** R-fb516b1e
- **Timestamp:** 2026-08-16T10:03:44Z
- **Catalogue:** v1.3-seed
- **Overall:** CONCERN
- **Needs Human:** no
- **Findings:** 1
...
- **AC#1 (Agent)** — What recall telemetry actually exists is established first and stated plainly — `lib/recall-usage.sh` and any retrieval log are read and their real coverage reported, including "there is no usable dat
  - **AC-verify-mismatch** (narrow, heuristic) — `path=lib/recall-usage.sh in: What recall telemetry actually exists is established first and stated plainly — `lib/recall-usage.sh` and any retrieval log are read and their real co`

### 2026-08-16T10:03:43Z — status-update [task-update-agent]
