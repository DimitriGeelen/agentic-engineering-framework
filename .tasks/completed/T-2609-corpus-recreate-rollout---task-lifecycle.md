---
id: T-2609
name: "Corpus recreate rollout - task-lifecycle and inception-flow via fw corpus prove
  with pre-flight loss guard"
description: >
  Roll the T-2605-proven identity-preserving recreate across the two remaining corpus
  maps carrying their own legacy-ref lint findings (aef-task-lifecycle, aef-inception-flow).
  First harden cmd_prove with a pre-flight round-trip check (generate from derived
  spec, canonical-diff vs snapshot IN MEMORY, refuse before any destructive call)
  — T-2605 only ever ran prove on maps already round-trip-proven; rollout targets
  are not. t2584-scratch's legacy-ref stays (fixture map, ghost-target by design).
