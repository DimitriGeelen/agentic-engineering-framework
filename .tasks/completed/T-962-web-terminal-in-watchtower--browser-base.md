---
id: T-962
name: "Web terminal in Watchtower — browser-based interactive terminal with TermLink
  integration, multi-session architecture for future orchestrator"
description: >
  Deep inception: embed interactive terminal in Watchtower web UI. Research OSS web
  terminal
  libraries (xterm.js, terminado, ttyd), Flask/WebSocket PTY integration, TermLink
  session
  convergence, multi-session UI patterns, security model, and orchestrator-aware design
  for
  future multi-agent/multi-provider routing. 7 parallel research vectors via TermLink
  dispatch.
