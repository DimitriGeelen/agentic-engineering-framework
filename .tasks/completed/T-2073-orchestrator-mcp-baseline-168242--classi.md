...
`fw audit` orchestrator-mcp-scan reports baseline 168 → current 242 (74 new unclassified MCP tools). Same convention as the 3 prior batches:

- **T-1755** (2026-05-06): +59 termlink_agent_* (chat/threads/reactions)
- **T-1760** (2026-05-06): +18 analytics primitives → 154
- **T-1867** (2026-05-15): +14 presence/streaks/silences/top-N → 168
- **T-2073** (this task): +74 new tools → 242

Classification rule (from baseline header): action-verb start = mutator_ungated; read-shape suffix = readonly_exempt. /opt/termlink is outside PROJECT_ROOT (path-isolation strict per memory `feedback_path_isolation_strict.md`); handler-level verification deferred per established baseline convention. Human corrects miscategorisations on review.

## Acceptance Criteria

### Agent
- [x] `.context/audits/orchestrator-mcp-baseline.yaml` baseline_count updated to 246 (242 from initial + 4 follow-on tools that surfaced on re-run) with 2026-05-28 update comment blocks matching T-1867's style.
- [x] 15 mutators added to `mutators_ungated` (11 channel-mutators + channel_poll_start + channel_poll_end + fleet_reauth + agent_send_auto_discover follow-on).
- [x] 61 read-shape tools added to `readonly_exempt` (58 initial + 3 follow-on: agent_listeners, agent_listeners_fleet, listener_heartbeat).
- [x] orchestrator-mcp-scan PASS — `Tools: 246 (baseline 246)`, 0 new unclassified.
- [x] baseline YAML parses cleanly — 246 unique tools verified via python3 + yaml.safe_load.
...
### 2026-05-28T17:47:08Z — task-created [task-create-agent]
