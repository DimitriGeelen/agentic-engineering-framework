---
id: T-3113
name: "L4: fw upgrade names which worktrees are behind (R7)"
description: >
  R7 leg 4. Vendored propagation is pull-only, so it must be loud. fw upgrade should
  report which of a project's worktrees carry a stale vendored framework, and doctor
  should say it unprompted. Carries the vendored fleet, since no other leg reaches
...
horizon: null
tags: []
components: [bin/fw, lib/hook_parity.py, lib/hook-parity.sh, lib/upgrade.sh, tests/unit/t3112_worktree_hook_parity.bats, tests/unit/t3113_upgrade_worktree_advisory.bats]
related_tasks: []
...
last_update: 2026-08-20T22:21:02Z
date_finished: 2026-08-20T22:21:02Z
...
# T-3113: L4: fw upgrade names which worktrees are behind (R7)
...
### 2026-08-20T17:37:22Z — task-created [task-create-agent]
...
- **Scan ID:** R-5d241780
- **Timestamp:** 2026-08-20T22:21:06Z
- **Catalogue:** v1.3-seed
- **Overall:** CONCERN
- **Needs Human:** no
- **Findings:** 1
...
- **AC#3 (Agent)** — For each linked worktree of `target_dir` it reports BOTH: how many commits behind the authority's HEAD the worktree is, AND its hook delta vs the authority's `.claude/settings.json`.
  - **AC-verify-mismatch** (narrow, heuristic) — `path=claude/settings.json in: For each linked worktree of `target_dir` it reports BOTH: how many commits behind the authority's HEAD the worktree is, AND its hook delta vs the auth`

### 2026-08-20T22:21:02Z — status-update [task-update-agent]
