---
id: T-2727
name: "fw init validates before it creates — the onboarding-task check can never run
  in the init path"
description: >
  fw init validates before it creates — the onboarding-task check can never run in
  the init path
...
date_finished: 2026-08-02T07:57:23Z
...
**Verification-level findings:**
  ...
  1. **l387-sigpipe-risk** (partial, heuristic) @ Verification:line 3
     - evidence: `d=$(mktemp -d); out=$(bin/fw init "$d" 2>&1); rm -rf "$d"; echo "$out" | sed 's/\x1b\[[0-9;]*m//g' | grep -q "func-tasks"`

- **Layer-1 escalations:** 1
  1. **destructive-action** (high) — Destructive operation in verification or AC
     - matched: `rm -rf`

### 2026-08-02T07:57:23Z — status-update [task-update-agent]
