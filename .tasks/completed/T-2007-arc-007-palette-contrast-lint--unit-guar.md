---
id: T-2007
name: "arc-007 palette-contrast lint — unit guard so no preset ships accent-ink/accent
  below WCAG AA (T-2006 prevention)"
description: >
  arc-007 palette-contrast lint — unit guard so no preset ships accent-ink/accent
  below WCAG AA (T-2006 prevention)
...
created: 2026-05-23T15:29:53Z
last_update: '2026-06-11T22:24:05Z'
date_finished: 2026-05-23T17:43:43+02:00
...
# T-2007: arc-007 palette-contrast lint — unit guard so no preset ships accent-ink/accent below WCAG AA (T-2006 prevention)

## Context

T-2006 fixed a sub-AA palette pair (linen accent-ink/accent 3.83:1) but its RCA flagged the
real gap: **S0 had no contrast gate** — token values were eyeballed against the design mock,
nothing computed `accent-ink/accent` against AA before they landed. The fix (one darker hex)
is mitigation; this task is the *prevention* (G-019: a gap isn't closed until recurrence is
structurally impossible). A pure-CSS-value unit test parses every palette in `foundations.css`
and asserts each `accent-ink/accent` pair clears WCAG AA — so the next palette edit that drops
a pair below AA fails `fw test unit` (and CI) at authoring, not in review. Cheaper and more
deterministic than the browser-driven ux-review check, which only fires in the review loop.
...
python3 -m pytest tests/unit/test_palette_contrast.py -q
...
### 2026-05-23T15:29:53Z — task-created [task-create-agent]
