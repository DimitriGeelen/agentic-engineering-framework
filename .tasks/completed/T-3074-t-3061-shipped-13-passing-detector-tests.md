...
T-3061 (closed 2026-08-17) added the unclosed-but-satisfied audit rail. Its work was
split between the parent session and a concurrent TermLink worker. The parent
committed `lib/task_satisfaction.py`, the scan wiring, and
`tests/unit/test_t3061_audit_wiring.py`. The worker's two bats files were never
`git add`ed:

- `tests/unit/t3061_unclosed_satisfied_scan.bats` — 8 tests, fixture-driven, pins
  the *rule* in both directions (qualifying task reported; unticked Agent AC,
  unticked Human AC, zero ACs, and template-comment-only examples each NOT reported).
- `tests/unit/audit_unclosed_satisfied_warn.bats` — 5 tests, integration-level, runs
  the real `audit.sh` section against a fixture project and pins WARN-never-FAIL.

**They are not redundant.** The tracked `test_t3061_audit_wiring.py` asserts the
*join* — that the scan calls the shared library and that `audit.sh` consumes the key.
These two assert the *rule* and the *audit integration*. Three different questions.

**Verified 2026-08-18:** all 13 pass, against the post-refactor code path (the rule
now lives in `lib/task_satisfaction.py`; the scan imports it). So they also serve as
independent confirmation that the T-3061 de-duplication preserved behaviour.

**Why this is a finding and not a chore.** An untracked test is worse than a missing
one. It passes locally for whoever wrote it, so the work *looks* covered; it is
absent from every clone, from `fw test`, and from lint; and one `git clean` deletes
it with no trace. The failure is silent in exactly the direction that matters — the
evidence of coverage exists on one machine and nowhere else.
...
### 2026-08-18T10:24:07Z — task-created [task-create-agent]
