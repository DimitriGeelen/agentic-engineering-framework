...
- [ ] `fw setup /tmp/test` runs interactive 6-step flow with breadcrumb headers
- [ ] Each step shows "Step N of 6: Title"
- [ ] Re-running on same directory skips completed steps
- [ ] `fw setup /tmp/test` in non-interactive mode applies defaults silently
- [ ] `fw doctor` passes after setup completes
- [ ] `.framework.yaml` contains all metadata fields after setup
