...
- [ ] `/project` response size measured off the wire before and after, and lands under the
      2,000,000-byte cap in `tests/playwright/test_all_routes_size.py`.
- [ ] The `/project` entry is removed from `KNOWN_OVER_CAP` in that guard. The guard fails a
...
- [ ] Worst case measured, not just the landing view. T-2775's paging looked done at 609 KB
