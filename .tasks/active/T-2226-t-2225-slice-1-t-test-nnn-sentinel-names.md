...
- [x] New helper fixture `tmp_project_root` defined in `web/test_app.py` (or `web/conftest.py`) that: (a) scaffolds `.tasks/{active,completed}` + `.context/episodic` under tmp_path, (b) monkeypatches BOTH `web.shared.PROJECT_ROOT` and `web.blueprints.tasks.PROJECT_ROOT`, (c) invalidates `_task_cache["data"] = None`. Verified by `grep -nE "def tmp_project_root|tmp_project_root\(" web/test_app.py` showing definition + uses.
