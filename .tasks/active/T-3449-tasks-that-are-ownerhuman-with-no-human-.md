...
# T-3449: Tasks that are owner:human with no Human criteria and all Agent criteria ticked have no route to closure — surface the class distinctly with per-task evidence and a one-command operator close

## Context

Found 2026-09-24 by the autonomous run's selection scan, then confirmed with the framework's
own verbs. A task that is `owner: human`, carries **no real `### Human` criteria** (only the
commented-out template), and has **every `### Agent` criterion ticked** can never close:

- `fw task delegate` (D-626, shipped by T-3445) declines it — measured on four candidates,
  all four printed `0 open Human criterion/criteria … converted 0, left human 0,
  owner: human (unchanged)`. The verb converts *deterministic Human criteria*; these have
  none to convert, so the delegation route the operator's ruling created does not reach them.
- Closing it directly is "completing a human-owned task", which agent authority forbids
  (CLAUDE.md §Autonomous Mode Boundaries, §Human Task Completion Rule).
- Nothing surfaces the class. `fw review-queue` puts them in a 316-row "Human ACs awaiting
  verification" list, indistinguishable from tasks that genuinely await judgement.

Measured instances (≥6, some months old): T-1274, T-1542, T-2205, T-2410, T-2420, T-3335.
CTL-029 correctly still fires on them — that narrowing (T-3444, 832's upstream) deliberately
preserved this branch as "the T-767 mis-filing", which is how the class surfaced.

**Out of scope, and deliberately so:** auto-closing any of them, and extending `fw task
delegate` to the zero-criteria case. A task with no open Human criteria *vacuously* satisfies
"all open criteria are deterministic", and acting on vacuous truth is exactly what should be
...
- [ ] A predicate (in `lib/`, importable and unit-tested) classifies an active task as
      ...
      during discovery); at least one `### Agent` criterion and all of them ticked. Unit tests
      cover: the template-comment false positive, a task with one real Human criterion (not in
...
### 2026-09-24T17:34:12Z — task-created [task-create-agent]
