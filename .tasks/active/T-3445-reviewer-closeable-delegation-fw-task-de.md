...
last_update: '2026-09-23T11:47:27Z'
date_finished:
...
### Agent
- [ ] `lib/delegation.py` (or bash + embedded python matching neighbours) classifies one
      ...
      `act-in-the-world` (publish, deploy, release, send, post to an external service, pay,
      delete remote state), `tier0-or-bypass` (Tier 0 approval, `--force`, `--skip-*`,
      `FW_ALLOW_*`), `sovereignty-field` (`bvp confirm`, arc close/approve-driver/abandon,
      ownership change), `render-surface` (task components or diff touch `web/templates`,
      `web/static`, `web/blueprints`, `web/shared.py`, `web/app.py`). Unit tests: at least two
...
- [ ] `fw task delegate T-XXX [--dry-run]` on a `### Human`-bearing task: prints one line per
      Human criterion with its class; converts every `deterministic` one to a `### Agent`
      criterion prefixed `[REVIEWER]` keeping Steps/Expected/If-not verbatim; leaves every
      carve-out criterion under `### Human` untouched; adds
      `bin/fw reviewer T-XXX 2>&1 | grep -q "Overall:.*PASS"` to `## Verification` once; sets
      `owner: agent` only when no Human criterion remains, otherwise leaves `owner: human` and
      says so; writes a `## Updates` entry naming the ruling decision id, the converted
...
- [ ] Close path proven end to end on a fixture: a human-owned task with two deterministic
      ...
      criterion → `delegate` converts two, keeps one, owner stays human, close lands
      partial-complete. Both as bats, `TEST_TEMP_DIR` set, no bare `! grep -q`.
- [ ] `fw reviewer surface` (or `fw audit` section) reports the corpus counts
...
- [ ] CLAUDE.md §Human Task Completion Rule and §AC Classification Guidance gain one paragraph
      each citing the ruling decision id and the verb; `tests/lint/config-registry-parity.bats`
      stays green for the new key.
- [ ] Vendored copies synced for every touched file under `lib/ agents/ bin/ policy/`;
...
- [ ] Live proof, recorded in this task: `fw task delegate --dry-run` run over the 43
      human-owned above-median now-tasks (ids from the 2026-09-23 join) and the class table
      printed — how many become reviewer-closeable, how many stay human and why. No task is
      delegated for real in this task except the two fixtures; the real sweep is a follow-up
