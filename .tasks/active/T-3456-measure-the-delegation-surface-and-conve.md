...
Selected because the Q1 band is structurally agent-ineligible. Of the ten highest-ranked
Q1 tasks, nine are `owner: human` and `captured` (T-1265, T-1309, T-1685, T-2963, T-3276,
T-3331, T-3332, T-3333, T-3334); only T-2321 is agent-owned. "Work Q1 to exhaustion" has
almost nothing in it an agent may execute, so the highest-value agent-eligible work is
what *unblocks* Q1 rather than anything inside it.

### The surface, measured 2026-09-25 (AC 1)

`bin/fw reviewer surface` — reproduced rather than quoting the audit line:

```
reviewer-closeable 16, agent-self 1, operator-only 330
  (open Human criteria across 317 active tasks)
```

### Why the other 314 cannot convert (AC 3)

The audit line invites the reading "314 criteria are stuck". The class breakdown says
something quite different:

| class | count | share | verdict |
|---|---:|---:|---|
| **render-surface** | **188** | **54%** | legitimately human — no test settles layout (T-1766) |
| unclassified | 59 | 17% | the classifier could not read these |
| taste | 22 | 6% | legitimately human |
| sovereignty-field | 22 | 6% | legitimately human |
| deterministic | 16 | 5% | the entire convertible set |
| inception-decision | 16 | 5% | legitimately human |
| act-in-the-world | 12 | 3% | legitimately human |
| tier0-or-bypass | 11 | 3% | legitimately human |
| agent-self | 1 | — | wrong audience (T-2143) |

**Over half the operator's review queue is "look at a web page."** 83 more are genuinely
sovereign. So the honest total of *legitimately* operator-only work is ~271, and the
delegation mechanism was never going to touch it — D-626 is not the lever on this queue.
The 59 unclassified are the only real uncertainty, and the only place mis-routing could
still be hiding at scale.

### The convertible set is not safe to convert as-is (AC 2, and why AC 4 parks)

`fw task delegate T-1701 --dry-run` (writes nothing) classified two criteria
**deterministic → REVIEWER** on the strength of the author's `[RUBBER-STAMP]` prefix:
...
An interactive `/login` against the operator's Anthropic account is act-in-the-world in
the sense that matters: **the agent cannot perform it and a static scan cannot verify it
happened.** Converted to `[REVIEWER]`, the T-1985 auto-tick rail could then tick it as
satisfied when nobody logged in — a false green on the operator's own credentials.

The precedence is not the bug. `lib/delegation.py:440-456` checks act-in-the-world (5)
before deterministic (7), exactly as documented. The bug is the **vocabulary**:
`_ACT_IN_THE_WORLD_RE` (:173) covers outbound irreversible acts — publish, deploy,
release, push, pay, delete-remote — and contains nothing for installing software on the
operator's host or authenticating with their credentials.

Measured across the whole corpus with the shipped classifier:

```
deterministic criteria (would convert to REVIEWER): 16
of those mentioning install / login / credentials:   3
...
**3 of 16 — 19% of the entire convertible set.** Converting the delegable list as it
stands would mis-route nearly a fifth of it. AC 4 says conversion is evidence-led; the
evidence says do not convert until the classifier is fixed. Filed as its own task per
one-bug-one-task; this task parks on it rather than converting around it.

## Acceptance Criteria

### Agent
<!-- Criteria the agent can verify (code, tests, commands). P-010 gates on these. -->
- [x] **The surface is measured, not quoted.** `fw reviewer surface` (or the equivalent
      corpus scan) is run here and its counts recorded with a date: how many open Human
      criteria exist, how many are reviewer-closeable, how many are operator-only, over how
      many active tasks. The audit's "330 operator-only / 16 reviewer-closeable" line is a
      starting hypothesis, not evidence — it is reproduced or corrected.
- [x] **The 330 are classified, not just counted.** `fw task delegate --dry-run` is run
      across a defined sample and the class of every open Human criterion recorded, grouped
      by the classifier's verdict. The output distinguishes the six never-converted
      carve-outs (taste, inception-decision, act-in-the-world, tier0-or-bypass,
      sovereignty-field, render-surface) from criteria that are genuinely deterministic.
      **`--dry-run` writes nothing** — this criterion is read-only by construction.
- [x] **The gap between "reviewer-closeable" and "operator-only" is explained.** If the
      classifier says only ~16 of ~330 can convert, say *why* the other ~314 cannot, by
      class and with counts. The interesting answer is whether they are genuinely human
...
- [x] **Whatever is found is left navigable.** The measurement is written somewhere the
      operator can act on without re-deriving it — counts by class, and the specific task
      IDs behind the convertible set — rather than living only in this task's prose.
...
### 2026-09-25T07:06:17Z — task-created [task-create-agent]
