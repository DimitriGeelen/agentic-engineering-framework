---
id: T-2563
name: "designer-corpus D4: dispatch-orchestration loop process diagram (resolver dispatch → worker → outcome backprop)"
description: >
  designer-corpus D4: dispatch-orchestration loop process diagram (resolver dispatch → worker → outcome backprop)
...
last_update: 2026-07-19T20:54:52Z
date_finished: null
...
- [ ] Diagram `aef-dispatch-loop` drafted in 832's canonical dialect covering resolver dispatch → worker execution → outcome evaluate/backprop, including the paused-dispatch sovereignty step (operator `fw pause resolve`) and at least one honest `kind=message` typed event (bus/inbox coupling)
- [ ] Saved through the LIVE designer gallery API (`POST /api/save`, id=aef-dispatch-loop) — meta.json + v1.bpmn exist under `.context/designer/projects/aef-dispatch-loop/`
- [ ] `fw bpmn compile` on the saved v1.bpmn exits 0; every expected WARN class accounted for in the verbatim compile log at `docs/reports/T-2563-d4-compile-log.md`; any NEW gap class filed as its own arc-014 task, not fixed mid-flight
- [ ] Owner derivation correct: sovereignty-lane nodes (pause resolve) → owner human; initiative-lane nodes → owner agent
