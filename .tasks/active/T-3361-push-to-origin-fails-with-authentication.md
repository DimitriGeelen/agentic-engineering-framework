---
id: T-3361
name: "push to origin fails with Authentication failed — 39 commits unpushed"
description: >
...
cost_estimate_proposed:
  - ts: '2026-09-11T20:30:10Z'
    ...
    rationale: blast_radius=? (no-components-UNMEASURED-not-zero); tier=2 
      (workflow:build); effort=8 (lines=312,acs=7)
    rubric_sha: e4a00f38e801
bvp_scores_proposed:
  - ts: '2026-09-11T20:30:21Z'
    estimator: bvp-estimator-v1-heuristic
    scores:
      D1: 4
      D2: 4
...
# T-3361: push to origin fails with Authentication failed — 39 commits unpushed

## Context

<!-- One sentence for small tasks. Link to design docs for substantial ones. -->

## Findings (2026-09-11) — BLOCKED on an operator-held secret

Evidence gathered, none of it conclusive between the two candidate causes:

- `git credential fill` for the OneDev host returns a credential **with a
  password present but an EMPTY username**.
- No username is configured anywhere: `git config --get-regexp credential` shows
  `credential.https://onedev…/provider generic` and no `.username`.
- **Read auth succeeds** — `git ls-remote origin HEAD` exits 0 (so does github).
- **Push is rejected** — `remote: You do not have permission to push to this
  project.` then `fatal: Authentication failed`.

**Correction to an earlier claim in this session.** The `git push --dry-run` that
appeared to prove the push path was open proved no such thing: `--dry-run` lists
refs and computes the would-be update locally, needing only READ access. It never
invokes receive-pack's write authorization. It did validly prove the *pre-push
audit gate* opened (`AUDIT-SCOPE: fails=0`) — that part stands, and is what
T-3357 fixed — but it was not evidence about push permission.

**Two causes fit the evidence identically and I cannot separate them:**

- **(a)** the empty username makes OneDev treat the request as anonymous —
  anonymous read is allowed on this project, anonymous push is not;
- **(b)** the stored token is expired or revoked, and the request falls back to
  anonymous with the same result.

Both need a value only the operator holds (the OneDev username, and/or a current
token). **Deliberately not attempted:** guessing usernames against a live auth
endpoint. That is credential brute-forcing in all but name, risks lockout, and
would at best confirm (a) by luck.

`mcp__skills__credentials_list` was tried as the sanctioned lookup path and hung
...
bash -c 'set -o pipefail; out=""; ok=0; for _i in 1 2 3 4 5 6 7 8; do out=$(git push origin bleeding-edge --dry-run 2>&1); if echo "$out" | grep -qE "Everything up-to-date|-> bleeding-edge"; then ok=1; break; fi; echo "$out" | grep -q "Authentication failed" && break; sleep 15; done; [ "$ok" = "1" ] && ! echo "$out" | grep -q "Authentication failed"'
...
### 2026-09-11T20:17:04Z — task-created [task-create-agent]
