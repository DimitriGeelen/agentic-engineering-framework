...
last_update: 2026-07-19T20:46:51Z
date_finished: null
...
- [ ] Diagram `aef-session-lifecycle` drafted in 832's canonical dialect (aef:workflowMeta schemaVersion=2, laneMeta authority=, uid attribute form, aef:position) covering init → work loop → budget escalation → handover → push-verify, with at least one typed timer event (budget/cron character) and the commit-cadence loop edge
- [ ] Saved through the LIVE designer gallery API (`POST /api/save`, id=aef-session-lifecycle) — meta.json + v1.bpmn exist under `.context/designer/projects/aef-session-lifecycle/`
- [ ] `fw bpmn compile` on the saved v1.bpmn exits 0; every expected WARN class (typed-event T-2551, gateway T-2557) accounted for in the verbatim compile log at `docs/reports/T-2561-d3-compile-log.md`; any NEW gap class filed as its own arc-014 task, not fixed mid-flight
- [ ] Owner derivation correct: sovereignty-lane nodes → owner human; initiative-lane nodes → owner agent
