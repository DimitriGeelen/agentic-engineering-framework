...
description: >
  Map the exception-handling process (detection → classification → escalation routing per Error Escalation Ladder → resolution → learning capture; lib healing + status:issues flow) as a corpus map + conformance rail. Second of the package's four worst-regression processes (T-2662 gap 6). Gated on the tier0-escalation P4 test outcome — promote horizon only after that lands.
...
last_update: 2026-07-28T16:19:32Z
date_finished: null
