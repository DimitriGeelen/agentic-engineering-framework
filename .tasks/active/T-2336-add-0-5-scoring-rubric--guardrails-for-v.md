...
Operator accepted the 3 V_* drivers via the T-2332 propose-queue and
immediately surfaced two follow-up gaps:

1. **No scoring rubric/hover-tooltip on V_***: D1-D4 + F-RECALL + F-ORCH
   all have `rubric: {0..5: "..."}` + `guardrails:` populated; `/bvp`
   renders a "?" hover with the rubric. V_* landed via `fw bvp driver --add`
   which only sets name/weight/rationale — rubric is missing.
2. **Weight-history pollution**: `tests/playwright/test_bvp_form_htmx.py`
   added `t2079probe` to F1+F2 across two pytest runs; cleanup curl ran
   but no remove entries appeared in `.context/bvp-weight-history.yaml`.
   Orphans cleaned in same commit.
