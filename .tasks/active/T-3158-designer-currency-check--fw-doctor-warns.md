---
id: T-3158
name: "Designer currency check — fw doctor warns when a newer designer-v* tag exists
  than the pin"
description: >
...
cost_estimate_proposed:
  - ts: '2026-08-26T12:00:09Z'
    ...
    rationale: blast_radius=? (no-components-UNMEASURED-not-zero); tier=2 
      (workflow:build); effort=8 (lines=231,acs=10)
...
# T-3158: Designer currency check — fw doctor warns when a newer designer-v* tag exists than the pin

## Context

T-3157 re-pinned the designer 0.8.0 → 0.11.0 after **four weeks** of three unconsumed
releases. Nothing on our side reported it, and the reason is a false green:
...
That line is TRUE. It measures **integrity** — do the vendored bytes match the pin — and it
is read as **currency**. The two questions are different and we only had an instrument for
one. 832 registered the producer half as their G-024 ("no instrument holds the pinned
artifact and src at the same time"); this is the consumer half: *is the release ahead of my
pin?*

001-CashWeb hit the identical gap, built `scripts/check-designer-currency.py` (~150 lines,
no AEF-internal dependencies, reads only `version:` and `source_origin:` from the pin), and
**offered it to us twice on the rail**. Reimplementing it would be worse manners and worse
engineering. Ask for the file; adopt with attribution.

**Why the four control legs are ACs and not nice-to-haves.** 001-CashWeb named the failure
mode precisely: *a checker with an always-warn bug passes "warns when the pin is stale"
IDENTICALLY to a correct one.* Leg 1 (current pin → silent) is the only leg that
discriminates. This is the same discipline as our own `[control]` mutation tests and the
T-3147 coverage control — a fixture that cannot go red measured nothing.
...
### 2026-08-26T11:57:11Z — task-created [task-create-agent]
