...
components: [web/blueprints/arcs.py, web/templates/arc_close.html]
related_tasks: [T-1902, T-1671, T-1668, T-1626, T-1909, T-1910]
arc_id: arc-005
created: 2026-05-18T21:30:46Z
last_update: 2026-05-20T14:33:21Z
...
WT_URL=$(bin/fw watchtower url 2>/dev/null || echo "http://localhost:3000"); out=$(curl -sf "$WT_URL/arcs/arc-grooming/close" 2>&1); grep -q 'name="demo_mode"\|demo_mode' <<<"$out"
WT_URL=$(bin/fw watchtower url 2>/dev/null || echo "http://localhost:3000"); out=$(curl -sf "$WT_URL/arcs/arc-grooming/close" 2>&1); grep -q 'headline_mechanic\|Headline mechanic' <<<"$out"
...
out=$(bin/fw test playwright tests/playwright/test_arc_close.py 2>&1); grep -qE "[0-9]+ passed" <<<"$out"
...
### 2026-05-18 — keep the gate, ship the form

- **What changed:** Initial design temptation was to add a Python re-implementation of demo/justification validation inside the Watchtower handler — faster to render errors inline. Built it that way, then realised the canonical contract lives in `lib/arc.sh:arc_close` (the §ACD agent-gate, T-1671) and forking validation would create a two-source-of-truth class. Refactored to shell-out via `subprocess.run` with `--from-watchtower` and propagate stderr to the form.
- **Plan impact:** Backend is thinner than originally scoped (no validation logic in Python — just envelope + shell call). UI complexity moved to JS for the disable-submit-when-demo=none-and-justification-short gate, which is cheap.
- **Triggered:** No follow-up needed; the pattern (browser submits → handler shells to fw via `--from-watchtower` exemption → errors render inline) is now the template for other agent-blocked verbs that need a Watchtower surface.

### 2026-05-18 — §ACD prompt rendered as part of the form

- **What changed:** Original spec had §ACD as a checklist the human reads before submitting (out-of-band). During build it became clear that surfacing the three §ACD questions *inside* the close form (above the demo/decision fields) is the structural counter to substrate-vs-deliverable conflation — the human cannot submit without scrolling past the question "does the captured `--demo` artefact show the headline_mechanic firing".
- **Plan impact:** Added a §ACD prompt block to the close template; no new backend logic. The render is text-only — answering the questions is left to the human's judgment, not gated programmatically (gating would re-create the false-positive class G-062 was trying to eliminate).
- **Triggered:** No follow-up.
...
- **Scan ID:** R-1bcdd4a6
- **Timestamp:** 2026-05-20T14:33:46Z
- **Catalogue:** v1.3-seed
- **Overall:** PASS
- **Needs Human:** no
- **Findings:** none

### 2026-05-20T14:33:21Z — status-update [task-update-agent]
