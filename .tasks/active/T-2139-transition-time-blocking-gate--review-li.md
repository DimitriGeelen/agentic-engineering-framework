...
last_update: 2026-05-31T13:28:30Z
date_finished: 2026-05-31T13:28:30Z
...
### Human
- [ ] [REVIEW] The validator's error message reads well enough that an agent who hits it will fix the task instead of fight the gate.
  **Steps:**

  **1. Context — what this AC is asking.**

  T-2139 ships a gate. When an agent tries to hand a task off for review and the Steps section tells the reviewer to *construct their own URL* instead of giving a literal `http://...` link, the gate refuses the handoff and prints an error block on the agent's terminal. The agent reads that error, replaces the construct-it-yourself instruction with concrete URLs, retries.

  This AC is about the **wording of that error block**. Four facts about it are already confirmed by the agent (AC8-AC11, all ticked): it names the task class, gives the right URL route, names both bypass mechanisms. **The only thing left to judge is whether the text reads well** — i.e. whether a tired agent reading it at 2 AM will understand what to do, or get defensive and reach for the bypass flag without thinking.

  **2. What you are looking at below.**

  Two real captures of the error block, one for each task class the gate handles. They are identical except for the URL-route line in the middle (inception → `/inception/T-…`, build → `/review/T-…`). The captures were generated by feeding a synthetic task containing the homework pattern through `python3 lib/review_link_validator.py --enforce`, the exact code path an agent hits.

  **3. How to evaluate.**

  Read both renderings as if you are the agent who just got blocked. For each rendering ask three questions:

    - **Clear?** Do I understand which specific anti-pattern I tripped?
    - **Actionable?** Do I know what to type next to make this go green?
    - **Tone?** Does it read like a teammate flagging an issue, or like a system rejecting me?

  **Expected (= what PASS looks like):**

  All three questions answer "yes" for both renderings. The verbs read instructional ("Replace…", "go to…", "Bypass: …") rather than punitive ("ERROR", "REFUSED", "INVALID"). The two bypass lines feel like a partnership offer ("here's how to escape if you really need to") rather than a loophole disclosure. Tick the checkbox.

  **If not (= what FAIL looks like):**

  Any phrase reads condescending, jargon-y, or makes you want to fight the gate. Leave the checkbox unticked and name the phrase in the review comment (most likely candidates if FAIL: the bare word `BLOCK` at the top might land too clinical; the term `homework` might land condescending; the `NOT /review/T-XXXX` clause might read as a finger-wag rather than a useful contrast). The agent will then rework the strings in `lib/review_link_validator.py`, re-capture both renderings, and you re-review.

  **4. What happens after.**

  - **PASS** (tick) → This is the last unticked AC on T-2139. Ticking it moves the task to `completed/` automatically and closes the T-2138 V1 keystone slice.
  - **FAIL** (leave unticked + comment) → Agent reworks the validator's strings, re-runs synthetic captures, updates the two renderings below, and pings you to re-evaluate. No re-doing the structural work — only the wording.

  ---

  **Rendering 1 — Inception task. This is what an agent sees on stderr when the gate fires on an inception:**
  ...
  **Rendering 2 — Build task with unticked Human ACs (partial-complete). This is what an agent sees on stderr when the gate fires on a build:**
...
- **Scan ID:** R-18743c4c
- **Timestamp:** 2026-05-31T13:28:37Z
- **Catalogue:** v1.3-seed
- **Overall:** CONCERN
- **Needs Human:** no
- **Findings:** 1
...
**Verification-level findings:**
  ...
  1. **mock-only-integration** (partial, heuristic) @ AC vs Verification cross-check
     - evidence: `python3 -m pytest tests/unit/test_review_link_validator.py -q`
