...
last_update: '2026-07-21T08:45:08Z'
date_finished:
...
### Agent
- [ ] aef-task-lifecycle v2.bpmn adds an intermediateThrowEvent with `aef:link targetWorkflow="aef-dispatch-loop"` wired into the flow at the dispatch step; meta.json bumps latest to 2 with a pair-draft note
- [ ] aef-inception-flow v2.bpmn adds an intermediateThrowEvent with `aef:link targetWorkflow="aef-task-lifecycle"` wired after the GO branch; meta.json bumps latest to 2 with a pair-draft note
- [ ] `bin/fw bpmn compile` on both v2 files exits 0 with ZERO off-page WARNs (both targets resolve — T-2570 silent-resolve discipline)
- [ ] Registry rescan shows no new ghosts (resolved refs must not ghost)
- [ ] Live: /designer opens each v2 (latest) and double-click on the handoff node opens the target workflow (verified via Playwright against :3001)
