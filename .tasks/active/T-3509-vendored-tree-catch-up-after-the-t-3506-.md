...
The T-3506 batch merge landed source changes without vendoring them, so the
pre-push self-vendor gate refuses the push. Three vendored files are stale, and in
every case **the source side is clean and committed** — this is catch-up, not
shipping anyone's unfinished work:

| vendored file | source state |
|---|---|
| `.agentic-framework/agents/git/lib/bypass.sh` | clean (from `dispatch-f25`) |
| `.agentic-framework/lib/init.sh` | clean (from `dispatch-f21-f24`) |
| `.agentic-framework/lib/bvp.sh` | clean (T-3487's authorised confirm-waiver half) |
| `.agentic-framework/VERSION` | stamp |

`policy/value-drivers.yaml` is the only **dirty** source file in the tree — another
worker's in-flight edit. It must stay withheld; the vendor rail does that on its own
and this task must not override it with `FW_VENDOR_ALL=1`.

This task exists because of **OBS-250**: closing a task clears `focus.yaml`, and
`fw vendor self` is refused with focus null, so a just-closed task's trailing vendor
sync has no task to run under. Hit for the fourth time this session. Filing a task to
run one command is the workaround CLAUDE.md names as a dead end — recorded here
...
### 2026-09-26T19:39:11Z — task-created [task-create-agent]
