...
### Agent
- [ ] `tests/integration/upgrade_live_simulation.bats` (or equivalent path) exists and contains at least one bats test that (a) builds a minimal docker image with the vendored consumer skeleton, (b) runs `fw upgrade` against it without `--dry-run`, (c) asserts post-upgrade `.agentic-framework/VERSION` advanced and `bin/fw` was rewritten
- [ ] Test runs in ≤5 min wall-clock on the framework dev host (timing recorded in the test output)
- [ ] Test is wired into `fw test integration` (or wherever release-gate tests run) so CI/release flow exercises it
- [ ] Failure modes documented: (a) docker not available → SKIP cleanly with explanatory message, (b) network unavailable → SKIP, (c) live upgrade mutates state but verification fails → FAIL with reproducible artifact
- [ ] Closes F3 in `docs/reports/T-2078-fw-upgrade-reliability-review.md` — verified by grep on the report's Status column flipping F3 to "shipped"
