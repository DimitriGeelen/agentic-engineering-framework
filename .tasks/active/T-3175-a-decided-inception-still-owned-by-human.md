---
id: T-3175
name: "A decided inception still owned by human appears on no review queue — /approvals and fw review-queue both drop it"
description: >
  A decided inception still owned by human appears on no review queue — /approvals and fw review-queue both drop it
...
last_update: 2026-08-26T15:40:57Z
date_finished: null
...
- [ ] A predicate exists that selects inceptions in `.tasks/active/` with a recorded
      `**Decision**: GO|NO-GO|DEFER` AND `owner: human` AND status ≠ `work-completed`,
      and it returns T-3159, T-3149 and T-3097 on today's corpus
- [ ] `/approvals` renders that set in a named section, distinct from the
      pending-decision "Decisions" section (a decided item is a different operator
      action from an undecided one)
- [ ] `fw review-queue` returns the same set as `/approvals` — the two surfaces share
      one predicate rather than each carrying their own
- [ ] A test pins the empty-candidate-set failure: seed a decided+human-owned inception
