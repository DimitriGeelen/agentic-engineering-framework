...
## Sovereign Question (blocks closure — do not self-resolve)

**Question.** T-3363's AC 3 reads "All 16 previously-contaminated tests pass."
Measurement shows the task premise was wrong: the 16 have **two** independent
causes, and this task's fixture closes only the first (6 of 16). The other 10 are
a different mechanism entirely and are now filed as **T-3367**.

Two defensible resolutions, and the choice is a scope decision, not a technical one:

- **(a) Narrow AC 3 to cause 1** — "the 6 cause-1 victims pass" — and close T-3363
  now, with cause 2 tracked under T-3367. Matches one-bug-one-task: the two causes
  are independent bugs with different root causes, fixes, and regression tests.
  Risk: rewriting an acceptance criterion after seeing the result is the exact
  shape of grading your own work upward, even when the rewrite is defensible.
- **(b) Leave T-3363 open until T-3367 lands**, then verify all 16 together
  against the original AC. Keeps the criterion untouched and independently
  checkable. Risk: one task's closure now depends on another's, and T-3363's own
  deliverable is finished, committed, and measured.

**Why I am not deciding this.** Option (a) requires me to edit the standard my own
output is judged against, after seeing that my output does not meet it. That is
the producer-judging-itself boundary regardless of how reasonable the edit looks.
Recorded for the Sovereign; task parked.
