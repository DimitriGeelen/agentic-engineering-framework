---
id: T-3247
name: "restart relaunches headless claude with no prompt, so the loop resumes into
  a dead session"
description: >
  claude-fw:551 sets CLAUDE_ARGS=() on the budget restart (T-3166, fresh session not
  -c). Correct for an interactive terminal, where SessionStart injects the directive
  and the operator's session picks it up. In headless -p mode the relaunched claude
  has no prompt, prints 'Input must be provided either through stdin or as a prompt
  argument when using --print' and exits 1 immediately. The loop therefore restarts
...
# T-3247: restart relaunches headless claude with no prompt, so the loop resumes into a dead session
...
### 2026-09-01T19:58:10Z — task-created [task-create-agent]
