...
last_update: '2026-07-05T16:45:05Z'
date_finished:
...
### Agent
- [ ] `fw doctor` (and/or audit) emits a WARN when the current working branch has BIDIRECTIONALLY diverged from origin/master (host ahead ≥1 AND host behind > threshold) — distinct from the existing "behind" WARN; reuses/extends the `FW_BRANCH_BEHIND_WARN` config surface (new `FW_BRANCH_DIVERGED_WARN` or a `diverged` sub-check)
- [ ] The WARN message names the safe reconciliation path (not "run git merge") — points at `fw integrate` / ff-only / union resolver per the leg-2 decision
- [ ] bats coverage: a fixture repo with a forked branch triggers the diverged-WARN; a merely-behind branch triggers only the existing behind-WARN (no false diverged-WARN); an up-to-date branch triggers neither
- [ ] Leg 2 disposition recorded in `## Decisions`: either implemented here, or spun out as a referenced follow-up task/inception with its ID
