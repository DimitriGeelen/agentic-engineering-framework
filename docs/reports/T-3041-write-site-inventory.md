...
| 4 | `.context/dispatches.jsonl` | `lib/spawn.py:231-254` (`stamp_outcome`) | Full-file read → rewrite all rows → `tmp` + `os.replace` | Dispatch ledger, fleet-wide | The one RMW on a log that is otherwise append-only (`lib/resolver.py:813`). A dispatch appended between the read at `:231` and the `os.replace` at `:254` is **erased entirely**, not merely un-updated. This directly violates the invariant `lib/outcome.py:177` relies on ("NEVER touches dispatches.jsonl"). |
