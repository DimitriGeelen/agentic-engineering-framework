...
if [ "${STATUS_AGE}" -lt "$STATUS_MAX_AGE" ] || [ "$STATUS_LEVEL" = "critical" ]; then
    case "$STATUS_LEVEL" in
        ...
        critical)
            ...
            echo "══════════════════════════════════════════════════════════" >&2
            echo "" >&2
            echo "  Context is at ~$((STATUS_TOKENS * 100 / 200000))% of 200K." >&2
            echo "  Remaining context reserved for wrap-up." >&2
            echo "  Your task files already have all essential state." >&2
            echo "" >&2
...
case "$LEVEL" in
    ...
    critical)
        ...
        echo "══════════════════════════════════════════════════════════" >&2
        echo "" >&2
        echo "  Context is at ~$((TOKENS * 100 / 200000))% of 200K." >&2
        echo "  Remaining context reserved for wrap-up." >&2
        echo "  Your task files already have all essential state." >&2
        echo "" >&2
        echo "  ALLOWED: git commit, fw handover, reading files," >&2
