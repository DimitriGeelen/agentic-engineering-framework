#!/usr/bin/env bats
# T-2407: `fw verify-acs --arc <id>` filter (sibling of T-2405 review-queue --arc).
#
# Pins:
...
_write_pc_task() {
    # Build a task with status=work-completed + unchecked Human AC so verify-acs surfaces it.
...
@test "--arc arc-NNN form matches tasks tagged with that arc" {
    _write_pc_task "T-9001" "arc_id: orchestrator-rethink" ""
    _write_pc_task "T-9002" "arc_id: embeddings-strategy" ""
    run "$FW" verify-acs --verbose --arc arc-003
    # exit code reflects auto-PASS rate (existing semantic); the filter behaviour is in output.
...
@test "--arc slug form matches the same set as --arc arc-NNN" {
    _write_pc_task "T-9001" "arc_id: orchestrator-rethink" ""
    _write_pc_task "T-9002" "arc_id: embeddings-strategy" ""
    run "$FW" verify-acs --verbose --arc orchestrator-rethink
    [[ "$status" -eq 0 || "$status" -eq 1 ]]
    [[ "$output" == *"T-9001"* ]]
    [[ "$output" != *"T-9002"* ]]
}
...
@test "--arc matches arc_id: in arc-NNN form (not just slug)" {
    _write_pc_task "T-9003" "arc_id: arc-003" ""
    run "$FW" verify-acs --verbose --arc arc-003
    [[ "$status" -eq 0 || "$status" -eq 1 ]]
    [[ "$output" == *"T-9003"* ]]
}
...
@test "--arc matches legacy tags: [arc:<slug>] form (T-1849 back-compat)" {
    _write_pc_task "T-9004" "" "arc:orchestrator-rethink"
    _write_pc_task "T-9005" "" "arc:embeddings-strategy"
    run "$FW" verify-acs --verbose --arc arc-003
    [[ "$status" -eq 0 || "$status" -eq 1 ]]
    [[ "$output" == *"T-9004"* ]]
    [[ "$output" != *"T-9005"* ]]
...
@test "--arc unknown-id errors to stderr + exits 1" {
    _write_pc_task "T-9001" "arc_id: orchestrator-rethink" ""
    run "$FW" verify-acs --arc not-a-real-arc
    [ "$status" -eq 1 ]
