...
**Worker**:
A dispatched executor that runs a Delegation envelope. Three flavours: Task tool sub-agent (in-session, shares context), TermLink dispatched session (separate process, isolated context), or a non-Claude provider (`provider:openrouter` / `provider:llama` — schema slot exists in v1; adapters are v2 work). Always strictly downstream of the Agent. Workers do the substantive reasoning the Agent intentionally does not. **Envelope fidelity is full only for Claude flavours**: `allowed_tools`, `mcp_config`, and `permission_mode` map cleanly to Claude CLI flags but reduce to "model + prompt + cost_cap" for non-Claude providers — that asymmetry is intentional in v1 and will need a per-provider envelope-subset rule in v2.
