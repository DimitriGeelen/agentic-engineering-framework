...
**`[REVIEWER]` is necessary-but-not-sufficient on prose-quality ACs (T-1947, L-409):** The reviewer agent has nine detectors (tautology, empty-body, swallowed-errors, output-spoofing, empty-output-success, skip-as-pass, mock-only-integration, human-ac-mechanical-signal, ac-verify-mismatch) — **zero of them evaluate natural-language prose quality**. If your AC's Expected clause contains any of `{reads clearly, reads cleanly, reads accurately, tone, voice, unambiguous, actionable, intuitive, cleanly, rhythm, layout reads clean, fits the page flow, strikes the right tone, cohesive, feels right}`, the reviewer **cannot** verify it. Filing such an AC as `[REVIEWER]` causes the scanner to silently ignore it (no detector applies) while the verdict still reports findings on OTHER ACs — surface looks identical to "all clear on prose". This is a false-success class. Rule: prose-quality ACs are `[REVIEW]` (or paired — `[REVIEWER]` for the structural check AND a separate `[REVIEW]` for the prose). The static-scan detector `reviewer-prose-mismatch` (T-1947) emits CONCERN when this routing error appears, so the mis-class surfaces at task close instead of slipping past.
